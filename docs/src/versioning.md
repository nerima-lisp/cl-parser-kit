...
that is 290 symbols, each documented in the [API reference](api.md) and, since
`v1.0.0`, from the running image as well — `t/api-surface-test.lisp` enforces
both, so an undocumented export fails CI rather than shipping.
